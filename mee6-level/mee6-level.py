...
class Mee6Level(commands.Cog):
    ...
    async def level(self, ctx):
        """
        Shows recipient's MEE6 level.

        Gets stats from the main server even if modmail threads are in a secondary server.

        You can add a static message to include with the `set_level_msg` command.
        """
        ...
        if level is not None:
            ...
            embed = Embed(
                ...
                description=msg or Embed.Empty,
            )
        else:
            embed = Embed(
